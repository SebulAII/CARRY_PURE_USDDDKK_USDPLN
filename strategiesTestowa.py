...
class TestStrategy(backtrader.Strategy):
    ...
    def next(self):
        ...
        #wygładzanie
            ...
            today = len(self)
            if self.line.idx > self.warmup and today > self.pip_av_days and today > self.n_days:
                ...
                #średnia wygładzania od początku
                    ...
                    if self.ticket is None and self.ES_zmiany[-1] < (all_smooth_av * self.open_threshold) and self.carry_sum_daily > self.min_swap:
                        self.ticket = 1
                    ...
                    if self.ticket is not None:
                        ...
                        if self.ES_zmiany[-1] > (all_smooth_av * self.close_threshold) or (self.carry1+self.carry2) <0:
                            self.ticket = None
            ...
            if self.ticket is not None:
                ...
                self.pair1_carry.append((self.Pair1_swap_long * self.vol1 * self.datas[0].close[0])/self.datas[0].close[0])
                self.pair2_carry.append((self.Pair2_swap_short * self.vol2 * self.datas[1].close[0])/self.datas[1].close[0])
